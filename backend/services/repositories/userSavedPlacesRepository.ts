 ...
 */
const getAllByUser = async (userId: string): Promise<UserSavedPlace[]> => {
 ...
 */
const getById = async (id: string, userId: string): Promise<UserSavedPlace | null> => {
  const result = await sql`
    SELECT * FROM user_saved_places
 ...
 */
const getByLabel = async (userId: string, label: string): Promise<UserSavedPlace | null> => {
  const result = await sql`
    SELECT * FROM user_saved_places
 ...
 */
const create = async (data: UserSavedPlaceInsert): Promise<UserSavedPlace> => {
  // For home/work, check if already exists and update instead
  if (data.label === 'home' || data.label === 'work') {
    const existing = await getByLabel(data.user_id, data.label);
    if (existing) {
      return update(existing.id, data.user_id, {
        name: data.name,
 ...
 */
const deletePlace = async (id: string, userId: string): Promise<boolean> => {
  const result = await sql`
 ...
 */
const recordUse = async (id: string, userId: string): Promise<void> => {
  await sql`
    UPDATE user_saved_places
    SET
 ...
 */
const getFrequentPlaces = async (userId: string, limit: number = 5): Promise<UserSavedPlace[]> => {
  const result = await sql`
    SELECT * FROM user_saved_places
 ...
 */
const labelExists = async (userId: string, label: string): Promise<boolean> => {
  const result = await sql`
...
const bulkSync = async (
  userId: string,
  places: (UserSavedPlaceInsert & { updated_at?: string })[]
): Promise<UserSavedPlace[]> => {
  const synced: UserSavedPlace[] = [];

  for (const place of places) {
    const existing = await getByLabel(userId, place.label);
    ...
    if (!existing) {
      ...
      if (clientTime > serverTime) {
        ...
        const updated = await update(existing.id, userId, {
          name: place.name,

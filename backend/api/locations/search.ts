...
export default async function handler(req: VercelRequest, res: VercelResponse) {
  ...
  // Mode 3: Get safety POIs (police, hospitals, transit stations) within a viewport.
  if (mode === 'poi') {
    try {
      if (!bounds || typeof bounds !== 'string') {
        return res.status(400).json({ error: 'bounds parameter is required for poi mode (format: south,west,north,east)' });
      }
      const [south, west, north, east] = bounds.split(',').map(parseFloat);
      if (isNaN(south) || isNaN(west) || isNaN(north) || isNaN(east)) {
        return res.status(400).json({ error: 'Invalid bounds format' });
      ...
      const poiType = typeof req.query.type === 'string' ? req.query.type : undefined;
      const validTypes = new Set(['police', 'hospital', 'transit']);
      if (poiType && !validTypes.has(poiType)) {
        return res.status(400).json({ error: `Invalid type. Allowed: police, hospital, transit` });
      }
      ...
      const pois = poiType
        ? await sql`
            SELECT id, osm_id, type, subtype, name, address, phone,
  ...
  // Mode 4: Regular location search (default)
